...
async function buildCodexHeader() {
  ...
  const { installed, hasITXBridge, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasITXBridge, settings } = result.data;
  ...
  // Extract ITXBridge custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:ITXBridge-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract ITXBridge provider config
  const provider = settings?.models?.providers?.["itxbridge"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("itxbridge/") ? primary.replace("itxbridge/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasITXBridge, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use ITXBridge\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
