...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `ITXBridge (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[itxbridge] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[itxbridge] tray init error: ${err.message}\n`);
    return null;
  }
}
