# ITXBridge Architecture

_Last updated: 2026-02-06_

## Executive Summary

ITXBridge is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[ITXBridge Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.itxbridge/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.itxbridge/usage.json`, `~/.itxbridge/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.itxbridge/db.json`)
- usage stats: `~/.itxbridge/usage.json`
- request log lines: `~/.itxbridge/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[ITXBridge Runtime]
        Next[Next.js Server\nPORT=20127]
...
1. `usageDb` currently stores under `~/.itxbridge` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/itxbridge && npm run build`
- Build Docker image: `cd /root/dev/itxbridge && docker build -t itxbridge .`
