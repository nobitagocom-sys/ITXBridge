...
ITXBridge automatically routes your requests through the best available provider using a 3-tier fallback system. Never stop coding due to quota limits or rate limiting.

---

## How It Works

ITXBridge uses intelligent routing to maximize your existing subscriptions, minimize costs, and ensure 24/7 availability:

```
Request → ITXBridge → Check Tier 1 (Subscription)
                     ↓ quota exhausted
                     Check Tier 2 (Cheap)
                     ↓ budget limit
                     Check Tier 3 (Free)
...
ITXBridge monitors quota in real-time and switches providers automatically:
...
ITXBridge selects the best model based on:
...
When budget reached, ITXBridge automatically switches to free tier.
