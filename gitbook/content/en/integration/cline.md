...
Integrate ITXBridge with Cline VSCode extension to route your AI requests through ITXBridge's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- ITXBridge running locally or cloud endpoint configured
...
Set the base URL to your ITXBridge endpoint:
...
1. In the **Base URL** field, enter your ITXBridge endpoint
...
3. The key should start with `sk-itxbridge-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your ITXBridge configuration
...
4. Cline will use ITXBridge to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using ITXBridge
...
3. Get AI-powered explanations through ITXBridge

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use ITXBridge to understand context and make changes
...
1. Verify ITXBridge is running: `curl http://localhost:20127/health`
...
2. Verify your ITXBridge instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check ITXBridge logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use ITXBridge cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://itxbridge.com`
2. Make sure you have configured your API key in the ITXBridge cloud dashboard
...
## Integration with ITXBridge Features

### Model Routing

ITXBridge automatically routes your requests to the best available provider based on:
...
If a provider fails, ITXBridge automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through ITXBridge dashboard:
