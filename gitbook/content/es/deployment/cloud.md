...
Despliega ITXBridge en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.itxbridge` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/itxbridge
sudo chown $USER:$USER /var/lib/itxbridge
```
...
Crea `/etc/nginx/sites-available/itxbridge`:
...
sudo ln -s /etc/nginx/sites-available/itxbridge /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de ITXBridge
...
# Actualizar ITXBridge
cd /path/to/itxbridge/app
git pull
npm install
npm run build
pm2 restart itxbridge
...
tar -czf itxbridge-backup-$(date +%Y%m%d).tar.gz /var/lib/itxbridge

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/itxbridge-$(date +\%Y\%m\%d).tar.gz /var/lib/itxbridge
```
...
```bash
# Verificar si ITXBridge está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/itxbridge
chmod 755 /var/lib/itxbridge
```
