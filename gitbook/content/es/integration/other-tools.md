...
ITXBridge es compatible con cualquier herramienta que soporte el formato de API de OpenAI. Esta guía cubre patrones de integración genéricos para varias herramientas y aplicaciones personalizadas.

## Resumen

ITXBridge proporciona un endpoint de API compatible con OpenAI que funciona con:
...
Cualquier herramienta compatible con OpenAI puede conectarse a ITXBridge usando estas configuraciones:
...
Model: cualquier modelo de ITXBridge (cc/*, cx/*, glm/*, etc.)
```

**ITXBridge en la nube:**

```
Base URL: https://itxbridge.com/v1
API Key: your-api-key-from-dashboard
Model: cualquier modelo de ITXBridge (cc/*, cx/*, glm/*, etc.)
```
...
**Problema:** No se puede conectar a ITXBridge

```bash
# Verifica si ITXBridge está corriendo
...
- Verifica que ITXBridge esté corriendo
...
- Verifica la conexión de red a ITXBridge
