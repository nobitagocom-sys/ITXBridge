...
**ITXBridgeは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、ITXBridge自体は100%無料でオープンソースです。**
...
**ITXBridgeはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## ITXBridgeをセルフホストできますか?

**はい! ITXBridgeは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.itxbridge`)
...
**はい、ITXBridgeはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**

- すべてのデータは`~/.itxbridge`(またはカスタム`DATA_DIR`)にローカル保存
- ITXBridgeサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
- [CHANGELOG.md](https://github.com/decolua/itxbridge/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.itxbridge`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/decolua/itxbridge/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [itxbridge.com/docs](https://itxbridge.com/docs)
