...
ITXBridgeをOpenAI Codex CLIと統合し、OpenAI APIリクエストをITXBridgeのインテリジェントルーティングシステム経由でルーティングします。
...
1. ITXBridgeダッシュボードでAPIキーが正しいか確認
...
1. モデル名がITXBridge設定と一致するか確認
2. ITXBridgeダッシュボードでOpenAIプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにITXBridgeクラウドエンドポイントを使用するには:

```bash
export OPENAI_BASE_URL="https://itxbridge.com"
```

ITXBridgeクラウドダッシュボードでAPIキーが設定されていることを確認してください。
