...
ITXBridgeをCursor IDEと統合し、AIリクエストをITXBridgeのインテリジェントルーティングシステム経由でルーティングします。

## 前提条件

- Cursor IDEがインストール済み
- Cursor Proアカウント (カスタムAPIエンドポイントに必要)
- ITXBridgeクラウドエンドポイントが設定済み
- ITXBridgeダッシュボードからのAPIキー

## ⚠️ 重要な注意点

> **クラウドエンドポイントが必要**: Cursorは独自のサーバー経由でリクエストをルーティングし、localhostエンドポイントをサポートしません。ITXBridgeクラウドエンドポイント `https://itxbridge.com` を使用する必要があります。
...
Base URLをITXBridgeクラウドエンドポイントに設定:
...
3. ITXBridge設定からモデル名を入力 (例: `gpt-4`、`claude-opus-4-5` など)
4. **Add** をクリック
...
1. クラウドエンドポイントを使用しているか確認: `https://itxbridge.com`
2. インターネット接続を確認
3. ITXBridgeクラウドサービスが運用中か確認
4. VPNまたはプロキシが有効な場合は無効化してみる

### Localhostが動作しない

> **覚えておいてください**: Cursorはlocalhostエンドポイントをサポートしません。クラウドエンドポイント `https://itxbridge.com` を使用する必要があります。ローカルITXBridgeインスタンスを使用したい場合は、ngrokなどのトンネリングサービスを検討してローカルエンドポイントを公開してください。

## クラウドエンドポイントのセットアップ

ローカルでITXBridgeを実行し、Cursorで使用したい場合:

1. ITXBridge設定でクラウドエンドポイントを有効化
2. ITXBridgeダッシュボードでクラウドエンドポイントURLを設定
3. Cursor設定でクラウドURLを使用
4. ローカルITXBridgeインスタンスがインターネットからアクセス可能か確認

## ベストプラクティス

1. **モデルエイリアスを使用**: ITXBridgeで頻繁に使うモデル用のショートエイリアスを作成
