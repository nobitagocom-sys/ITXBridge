...
Tích hợp ITXBridge với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của ITXBridge.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- ITXBridge đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint ITXBridge:

**Cho ITXBridge cục bộ:**
...
1. Trong field **Base URL**, nhập endpoint ITXBridge
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key ITXBridge của bạn
2. Bạn có thể tìm API key trong ITXBridge dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-itxbridge-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình ITXBridge
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng ITXBridge để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua ITXBridge
...
3. Nhận giải thích AI qua ITXBridge

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng ITXBridge để hiểu context và thực hiện thay đổi
...
1. Xác minh ITXBridge đang chạy: `curl http://localhost:20127/health`
...
2. Xác minh ITXBridge instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs ITXBridge để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng ITXBridge cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://itxbridge.com`
2. Đảm bảo bạn đã cấu hình API key trong ITXBridge cloud dashboard
...
## Tích hợp với Tính năng ITXBridge

### Định tuyến Model

ITXBridge tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, ITXBridge tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua ITXBridge dashboard:
