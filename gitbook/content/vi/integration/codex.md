...
Tích hợp ITXBridge với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của ITXBridge.
...
ITXBridge cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong ITXBridge dashboard
...
1. Xác minh tên model khớp với cấu hình ITXBridge
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong ITXBridge dashboard
...
Đảm bảo bạn đã cấu hình API key trong ITXBridge cloud dashboard.
