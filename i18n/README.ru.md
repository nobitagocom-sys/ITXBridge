<div align="center">
  <img src="../images/itxbridge.png?1" alt="Панель управления ITXBridge" width="800"/>
  
# ITXBridge - Free AI Router
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  [![npm](https://img.shields.io/npm/v/itxbridge.svg)](https://www.npmjs.com/package/itxbridge)
  [![Downloads](https://img.shields.io/npm/dm/itxbridge.svg)](https://www.npmjs.com/package/itxbridge)
  [![License](https://img.shields.io/npm/l/itxbridge.svg)](https://github.com/decolua/itxbridge/blob/main/LICENSE)
  
  [🚀 Быстрый старт](#-quick-start) • [💡 Возможности](#-key-features) • [📖 Установка](#-setup-guide) • [🌐 Сайт](https://itxbridge.com)
</div>
...
Пакет этого репозитория приватный (`itxbridge-app`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
### 📺 Полное руководство по настройке - ITXBridge + Claude Code БЕСПЛАТНО
  
[![Настройка ITXBridge + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**

- ✅ Установка и настройка ITXBridge
...
ITXBridge бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → ITXBridge транслирует → Провайдер получает родной формат
...
> Сам ITXBridge **никогда ничего не взимает** с вас. Вы платите напрямую провайдерам (если используете платные сервисы).
>
...
### 📊 Понимание затрат и оплаты в ITXBridge

**Реальность оплаты ITXBridge:**

✅ **Софт ITXBridge = БЕСПЛАТНО навсегда** (открытый код, никогда не взимает плату)  
...
❌ **ITXBridge никогда не выставляет счёт** и не списывает с вашей карты
...
- **Дешёвые провайдеры** (GLM, MiniMax): Платите им напрямую, ITXBridge только маршрутизирует
- **БЕСПЛАТНЫЕ провайдеры** (iFlow, Kiro, Qwen): Действительно бесплатны навсегда, без скрытых платежей
- **ITXBridge**: Никогда ничего не взимает, никогда
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через ITXBridge.
...
<details>
<summary><b>💳 Взимает ли с меня плату ITXBridge?</b></summary>

**Нет.** ITXBridge — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**

- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, ITXBridge только маршрутизирует ваши запросы
- ❌ **Самому ITXBridge** → **Никогда ничего не взимает, никогда**

ITXBridge — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
ITXBridge только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а ITXBridge облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не ITXBridge.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - ITXBridge помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования ITXBridge предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и ITXBridge будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. ITXBridge отслеживает квоту для каждой модели!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [из панели управления itxbridge]
...
```json
{
  ...
  "anthropic_api_key": "your-itxbridge-api-key"
}
```
...
export OPENAI_API_KEY="your-itxbridge-api-key"
...
> **Примечание:** OpenClaw работает только с локальным ITXBridge. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
export DATA_DIR="/var/lib/itxbridge"
export PORT="20127"
export HOSTNAME="0.0.0.0"
export NODE_ENV="production"
export NEXT_PUBLIC_BASE_URL="http://localhost:20127"
export NEXT_PUBLIC_CLOUD_URL="https://itxbridge.com"
...
# Or use PM2
npm install -g pm2
pm2 start --name itxbridge -- start
pm2 save
pm2 startup
```
...
docker run -d \
  --name itxbridge \
  -p 20127:20127 \
  --env-file /root/dev/itxbridge/.env \
  -v itxbridge-data:/app/data \
  -v itxbridge-usage:/root/.itxbridge \
  itxbridge
```
...
```bash
docker logs -f itxbridge
docker restart itxbridge
docker stop itxbridge && docker rm itxbridge
```
...
|----------|---------|-------------|
| `JWT_SECRET` | Автогенерация (`~/.itxbridge/jwt-secret`) | Секрет подписи JWT для cookie аутентификации панели (задайте для общего доступа между инстансами) |
| `INITIAL_PASSWORD` | `123456` | Пароль первого входа при отсутствии сохранённого хеша |
| `DATA_DIR` | `~/.itxbridge` | Расположение основной БД приложения (`db.json`) |
...
| `CLOUD_URL` | `https://itxbridge.com` | Серверный базовый URL эндпоинта облачной синхронизации |
...
| `NEXT_PUBLIC_CLOUD_URL` | `https://itxbridge.com` | Обратно совместимый/публичный облачный URL (приоритет `CLOUD_URL` для серверного рантайма) |
...
- История использования и логи: `~/.itxbridge/usage.json` и `~/.itxbridge/log.txt`, управляется `src/lib/usageDb.js`.
- Опциональные логи запросов/транслятора: `<repo>/logs/...` при `ENABLE_REQUEST_LOGS=true`.
- Хранилище использования следует логике пути `~/.itxbridge` и независимо от `DATA_DIR`.
...
- Убедитесь, что `CLOUD_URL` указывает на ожидаемый облачный эндпоинт (например, `https://itxbridge.com`)
...
- Симптом обычно появляется на публичном облачном эндпоинте (`https://itxbridge.com/v1`) для непотоковых (non-streaming) вызовов.
...
- Локальный рантайм ITXBridge включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Отправляет OpenAI-совместимый запрос напрямую на облачный эндпоинт (`https://itxbridge.com/v1/chat/completions`) с указанной моделью/ключом.
...
- Публичный облачный эндпоинт (`https://itxbridge.com/v1/chat/completions`):
...
- **Сайт**: [itxbridge.com](https://itxbridge.com)
...
Спасибо всем, кто помогает делать ITXBridge лучше!

[![Contributors](https://contrib.rocks/image?repo=decolua/itxbridge&max=100&columns=20&anon=1)](https://github.com/decolua/itxbridge/graphs/contributors)

---

## 📊 Star Chart

[![Star Chart](https://starchart.cc/decolua/itxbridge.svg?variant=adaptive)](https://starchart.cc/decolua/itxbridge)
...
См. [Pull Requests](https://github.com/decolua/itxbridge/pulls) для подробных инструкций.

---

## 🔀 Форки

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Полнофункциональный TypeScript-форк ITXBridge. Добавляет 36+ провайдеров, авторезервирование на 4 уровнях, мультимодальный API (изображения, embedding, аудио, TTS), circuit breaker, семантическое кеширование, оценку LLM и доработанную панель. 368+ юнит-тестов. Доступен через npm.
