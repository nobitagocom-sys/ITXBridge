...
<div align="center">
  <img src="../images/itxbridge.png?1" alt="Bảng điều khiển ITXBridge" width="800"/>
...
# ITXBridge - Free AI Router
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://itxbridge.com)
</div>

---

## 🤔 Tại sao chọn ITXBridge?
...
**ITXBridge giải quyết vấn đề này:**
...
Gói kho lưu trữ này là riêng tư (`itxbridge-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - ITXBridge + Claude Code MIỄN PHÍ
  
[![Thiết lập ITXBridge + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**

- ✅ Cài đặt & thiết lập ITXBridge
...
ITXBridge hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → ITXBridge dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân ITXBridge **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
>
...
### 📊 Hiểu về Chi phí & Thanh toán của ITXBridge

**Thực tế Thanh toán ITXBridge:**

✅ **Phần mềm ITXBridge = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **ITXBridge không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, ITXBridge chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị ITXBridge tính phí không?</b></summary>

**Không.** ITXBridge là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**

- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, ITXBridge chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân ITXBridge** → **Không bao giờ tính phí bất cứ thứ gì, ever**

ITXBridge là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
ITXBridge chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và ITXBridge giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải ITXBridge.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - ITXBridge giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của ITXBridge ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và ITXBridge sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. ITXBridge theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển itxbridge]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với ITXBridge cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.itxbridge/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.itxbridge` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `https://itxbridge.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://itxbridge.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.itxbridge/usage.json` và `~/.itxbridge/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.itxbridge` và độc lập với `DATA_DIR`.
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://itxbridge.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://itxbridge.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime ITXBridge cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://itxbridge.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://itxbridge.com/v1/chat/completions`):
...
- **GitHub**: [github.com/decolua/9](https://github.com/decolua/itxbridge)
...
Cảm ơn tất cả những người đã đóng góp giúp ITXBridge tốt hơn!
...
[![ Chart](https://starchart.cc/decolua/itxbridge.svg?variant=adaptive)](https://starchart.cc/decolua/itxbridge)
...
Xem [Pull Requests](https://github.com/decolua/itxbridge/pulls) để biết hướng dẫn chi tiết.

---

## 🔀 Forks

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Một fork TypeScript đầy đủ tính năng của ITXBridge. Thêm 36+ nhà cung cấp, tự động dự phòng 4 tầng, API đa phương thức (hình ảnh, embedding, âm thanh, TTS), circuit breaker, bộ nhớ đệm ngữ nghĩa, đánh giá LLM và bảng điều khiển được tinh chỉnh. 368+ bài kiểm tra đơn vị. Có sẵn qua npm và.
