 ...
 */
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, sourceFormatOverride, providerThinking, sessionId, clientTool, userId }) {
  ...
  // Use clientTool passed from handleChat if available, otherwise detect locally
  const detectedTool = clientTool || detectClientTool(clientRawRequest?.headers || {}, body);
  ...
  if (passthrough) {
    ...
    if (detectedTool === "claude") normalizeClaudePassthrough(translatedBody, upstreamModel);
  } else {
    translatedBody = translateRequest(sourceFormat, targetFormat, upstreamModel, body, stream, credentials, provider, reqLogger, stripList, connectionId, detectedTool);
    if (!translatedBody) {
      trackPendingRequest(model, provider, connectionId, false, true, sessionMeta);
  ...
  const sessionMeta = sessionId ? { sessionId, clientTool: clientTool || null, userId: userId || null, connectionId } : null;
  trackPendingRequest(model, provider, connectionId, true, false, sessionMeta);
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, sessionId, clientTool };
  const appendLog = (extra) => appendRequestLog({ model, provider, connectionId, ...extra }).catch(() => { });
  const trackDone = () => trackPendingRequest(model, provider, connectionId, false, false, sessionMeta);

  // Provider forced streaming but client wants JSON
  if (!clientRequestedStreaming && providerRequiresStreaming) {
    const result = await handleForcedSSEToJson({ ...sharedCtx, providerResponse, sourceFormat, trackDone, appendLog, sessionId, clientTool });
  ...
  const { onStreamComplete } = buildOnStreamComplete({ ...sharedCtx, streamDetailId, sessionId, clientTool });
  return handleStreamingResponse({ ...sharedCtx, providerResponse, sourceFormat, targetFormat, userAgent, reqLogger, toolNameMap, streamController, onStreamComplete, streamDetailId, sessionId, clientTool });
}
