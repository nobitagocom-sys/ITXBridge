...
export default function JcodeToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (!jcodeStatus?.hasITXBridge) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["itxbridge"];
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["itxbridge"];
      if (provider) {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.itxbridge]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_ITXBRIDGE_API_KEY"
env_file = "provider-itxbridge.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.itxbridge.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;

    const envContent = `JCODE_ITXBRIDGE_API_KEY="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-itxbridge.env",
        content: envContent,
      },
    ];
  };
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {tool.notes && tool.notes.length > 0 && (
                  <div className="flex flex-col gap-2 mb-2">
                    {tool.notes.map((note, idx) => (
                      <div key={idx} className={`flex items-start gap-2 p-2 rounded text-xs ${note.type === "info" ? "bg-blue-500/10 text-blue-600 dark:text-blue-400" :
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["itxbridge"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile itxbridge</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile itxbridge --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.hasITXBridge} loading={restoring}>
