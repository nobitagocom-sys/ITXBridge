...
// Provider whitelist — only these are shown in each category
const WHITELIST = {
  oauth: new Set([]),
  free: new Set(["mimo-free", "opencode"]),
  freeTier: new Set(["openrouter", "nvidia"]),
  apikey: new Set(["alicode", "anthropic", "azure", "deepseek", "fpt-ai", "glm", "kimi", "openai", "ollama-local"]),
};
...
export default function ProvidersPage() {
  ...
  const oauthEntries = sortByPriority(
    Object.entries(OAUTH_PROVIDERS).filter(([id, info]) => !info.hidden && WHITELIST.oauth.has(id) && matchSearch(info.name)),
    "oauth",
  );
  const freeEntries = Object.entries(FREE_PROVIDERS)
    .filter(([id, info]) => !info.hidden && WHITELIST.free.has(id) && matchSearch(info.name))
    .sort(([, a], [, b]) => (b.noAuth ? 1 : 0) - (a.noAuth ? 1 : 0));
  const freeTierEntries = sortByPriority(
    Object.entries(FREE_TIER_PROVIDERS).filter(
      ([id, info]) =>
        !info.hidden &&
        WHITELIST.freeTier.has(id) &&
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:px-0">
      ...
      {/* Custom Providers (OpenAI/Anthropic Compatible) — hidden */}
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
              ...
              <button
                ...
                className={`flex w-full items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-medium transition-colors sm:w-auto sm:py-1.5 ${testingMode === "oauth"
              ...
              >
                <span
                  className={`${testingMode === "oauth" ? " animate-spin" : ""}`}
                >
                </span>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              disabled={!!testingMode}
              className={`flex w-full items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-medium transition-colors sm:w-auto sm:py-1.5 ${testingMode === "free"
                  ? "bg-primary/20 border-primary/40 text-primary animate-pulse"
                  : "bg-bg border-border text-text-muted hover:text-text-main hover:border-primary/40"
                }`}
            ...
            >
              <span
                className={`${testingMode === "free" ? " animate-spin" : ""}`}
              >
              </span>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              disabled={!!testingMode}
              className={`flex w-full items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-medium transition-colors sm:w-auto sm:py-1.5 ${testingMode === "apikey"
                  ? "bg-primary/20 border-primary/40 text-primary animate-pulse"
                  : "bg-bg border-border text-text-muted hover:text-text-main hover:border-primary/40"
                }`}
            ...
            >
              <span
                className={`${testingMode === "apikey" ? " animate-spin" : ""}`}
              >
              </span>
        ...
        >
          ...
          >
            <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3 border-b border-border bg-surface/95 backdrop-blur-sm rounded-t-xl">
              ...
              >
                <Icon name="close" size={18} className="text-lg" />
              </button>
            </div>
            <div className="p-5">
  ...
  if (results.error && !results.results) {
    return (
      <div className="text-center py-6">
        <Icon name="error" size={32} className="text-red-500  mb-2 block" />
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3">
      ...
      {items.map((r, i) => (
        ...
        >
          <span
            className={`${r.valid ? "text-emerald-500" : "text-red-500"}`}
          >
          ...
          <span
            className={`shrink-0 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${r.valid
