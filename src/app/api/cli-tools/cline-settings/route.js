...
const hasITXBridgeConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("itxbridge"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasITXBridge: hasITXBridgeConfig(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "ITXBridge settings removed from Cline" });
  } catch (error) {
