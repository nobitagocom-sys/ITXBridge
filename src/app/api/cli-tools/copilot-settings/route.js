...
const hasITXBridgeConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "ITXBridge");
};

const getITXBridgeEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "ITXBridge") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getITXBridgeEntry(config);
...
// POST - Apply ITXBridge config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing ITXBridge entry or append
    const idx = config.findIndex((e) => e.name === "ITXBridge");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove ITXBridge entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "ITXBridge");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "ITXBridge removed from Copilot config",
    });
  } catch (error) {
