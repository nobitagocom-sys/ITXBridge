...
// Check if settings has ITXBridge customModels
const hasITXBridgeConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:ITXBridge"));
};
...
// POST - Update ITXBridge customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing ITXBridge configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:ITXBridge"));
...
// DELETE - Remove ITXBridge customModels only (keep other settings)
