...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-itxbridge.env");
};
...
const hasITXBridgeConfig = (config) => {
  ...
  if (providers["itxbridge"]) return true;
...
export async function GET() {
  ...
  const hasITXBridge = hasITXBridgeConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_ITXBRIDGE_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile itxbridge",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["itxbridge"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_ITXBRIDGE_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "itxbridge configuration removed from jcode",
    });
  } catch (error) {
