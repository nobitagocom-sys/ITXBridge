...
const hasITXBridgeConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["itxbridge"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("itxbridge");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasITXBridge: hasITXBridgeConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "ITXBridge", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "ITXBridge settings removed from Kilo Code" });
  } catch (error) {
