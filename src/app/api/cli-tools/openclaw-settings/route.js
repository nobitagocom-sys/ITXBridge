...
// Check if settings has ITXBridge config
const hasITXBridgeConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["itxbridge"];
};

// Read per-agent models.json and return current model id (without "itxbridge/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["itxbridge"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasITXBridge: hasITXBridgeConfig(settings),
...
// POST - Update ITXBridge settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `itxbridge/${model}`;

    // Remove all old itxbridge/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("itxbridge/"))
    ...
    // Add fresh itxbridge models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`itxbridge/${m}`] = {};
    });

    // Remove old itxbridge model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("itxbridge/")) {
    ...
    // Update models.providers.itxbridge with all models
    settings.models.providers["itxbridge"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `itxbridge/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove ITXBridge settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove ITXBridge from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["itxbridge"];
    ...
    // Remove itxbridge models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("itxbridge/"));
    ...
    // Reset agents.defaults.model.primary if it uses itxbridge
    if (settings.agents?.defaults?.model?.primary?.startsWith("itxbridge/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "ITXBridge settings removed successfully",
    });
  } catch (error) {
