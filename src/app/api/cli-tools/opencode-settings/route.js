...
const hasITXBridgeConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["itxbridge"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["itxbridge"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasITXBridge: hasITXBridgeConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("itxbridge/") ? config.model.replace(/^itxbridge\//, "") : null,
...
// POST - Apply ITXBridge as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_itxbridge";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing itxbridge provider entry and its models
    const existingProvider = config.provider["itxbridge"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["itxbridge"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `itxbridge/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `itxbridge/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("itxbridge/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove ITXBridge provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["itxbridge"]?.models) {
      delete config.provider["itxbridge"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["itxbridge"].models).length === 0) {
        delete config.provider["itxbridge"];
        if (config.model?.startsWith("itxbridge/")) delete config.model;
      } else if (config.model === `itxbridge/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["itxbridge"].models);
        config.model = `itxbridge/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire itxbridge provider
      if (config.provider) delete config.provider["itxbridge"];
      if (config.model?.startsWith("itxbridge/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("itxbridge/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "ITXBridge settings removed from OpenCode",
    });
  } catch (error) {
