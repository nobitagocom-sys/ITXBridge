...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "ITXBridge", "Accept": "application/vnd.github+json" },
  },
  iflow: {
...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "fpt-ai": {
        const res = await fetchWithConnectionProxy("https://mkp-api.fptcloud.com/models", { headers: { Authorization: `Bearer ${connection.apiKey}` } }, effectiveProxy);
        return { valid: res.ok, error: res.ok ? null : "Invalid API key" };
      }
      case "deepseek": {
