...
// RFC 1918 private IPv4 ranges — treat as local (Docker bridge, LAN, etc.)
function isPrivateIpv4(ip) {
  const parts = ip.split(".");
  if (parts.length !== 4) return false;
  const b0 = parseInt(parts[0], 10);
  const b1 = parseInt(parts[1], 10);
  if (isNaN(b0) || isNaN(b1)) return false;
  // 10.0.0.0/8
  if (b0 === 10) return true;
  // 172.16.0.0/12 (covers Docker default bridge 172.17.0.0/16)
...
export function isLocalRequest(request) {
  ...
  if (realIp) {
    if (!isLoopbackHostname(realIp) && !isPrivateIpv4(realIp)) return false;
  ...
  if (origin) {
    try {
      ...
      if (!isLoopbackHostname(originHost) && !isPrivateIpv4(originHost)) return false;
    } catch { return false; }
  }
  return true;
}
