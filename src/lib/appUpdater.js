...
// Collect PIDs of all itxbridge-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from itxbridge install dir or wrapper cli.js
        const isAppProcess = lower.includes("itxbridge") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("itxbridge") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "itxbridge");
  }
  return path.join(os.homedir(), ".itxbridge");
}
...
// Resolve npx/itxbridge binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx itxbridge` — works regardless of global bin path changes after npm i -g
